@extends('layouts.admin', ['pageTitle'=>'Verifikasi Booking'])
@section('content')
<div class="space-y-5">
  <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 to-emerald-600 text-white shadow">
    ...
    <div class="relative grid md:grid-cols-5 gap-4 p-6 items-center">
      <div class="md:col-span-3 space-y-2">
        <div class="text-sm uppercase tracking-wide text-white/80">Panel Verifikasi Booking</div>
        <div class="text-2xl font-bold">Tinjau & setujui booking ruangan</div>
        <p class="text-white/80 max-w-xl">Filter cepat booking yang membutuhkan persetujuan takmir atau tinjau riwayat verifikasi yang sudah selesai.</p>
        <p class="text-white/75 text-sm">Gunakan tombol di bawah tabel untuk memilih status.</p>
      </div>
      <div class="md:col-span-2">
        <div class="bg-white/10 backdrop-blur rounded-xl border border-white/15 overflow-hidden shadow-lg">
          <img src="https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&w=900&q=80" alt="Ilustrasi verifikasi" class="w-full h-36 object-cover">
          <div class="p-4 text-sm text-white/90">
            <div class="font-semibold">Tip</div>
            <div class="text-white/80">Gunakan filter “Perlu Verifikasi” untuk fokus ke booking yang menunggu keputusan takmir.</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="{{ $cardBg }} p-5 rounded-xl shadow border {{ $tableAccent }} space-y-4">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <a href="{{ route('takmir.verifikasi-booking.index', array_merge($baseParams, ['mode'=>'waiting'])) }}"
           class="px-3 py-2 rounded-lg border border-amber-300 {{ $isWaiting ? 'bg-amber-200 text-amber-900' : 'bg-white/60 text-amber-900' }}">
          Perlu Verifikasi
        </a>
        <a href="{{ route('takmir.verifikasi-booking.index', array_merge($baseParams, ['mode'=>'verified'])) }}"
      ...
      <table class="min-w-full text-sm">
        ...
        @forelse($items as $p)
          <tr class="border-b {{ $rowTint }}">
            ...
            <td class="py-2 px-3">
              <a href="{{ route('takmir.verifikasi-booking.show',$p) }}" class="px-2 py-1 rounded bg-white border hover:bg-slate-50">Detail</a>
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="7" class="py-6 text-center text-gray-500">
              {{ ($mode ?? 'verified')==='waiting' ? 'Tidak ada booking yang perlu diverifikasi.' : 'Belum ada booking yang sudah diverifikasi.' }}
            </td>
          </tr>
        @endforelse
        </tbody>
