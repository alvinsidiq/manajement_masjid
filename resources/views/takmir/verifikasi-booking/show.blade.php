@extends('layouts.admin', ['pageTitle' => 'Detail Verifikasi Booking'])
@section('content')
@if(session('status'))
...
<div class="space-y-6">
    ...
    <div class="relative grid md:grid-cols-5 gap-4 p-6 items-center">
      <div class="md:col-span-3 space-y-2">
        <div class="flex items-center gap-2 text-sm uppercase tracking-wide text-white/80">
          <span class="px-2 py-1 rounded-full bg-white/15 border border-white/20">Verifikasi Booking</span>
      ...
      <div class="md:col-span-2">
        <div class="bg-white/10 backdrop-blur rounded-xl border border-white/15 overflow-hidden shadow-lg">
          <img src="https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=900&q=80" alt="Ilustrasi verifikasi booking" class="w-full h-36 object-cover">
          <div class="p-4 space-y-1 text-sm text-white/90">
            ...
            <div class="flex items-center justify-between">
              <span>Sumber</span><span class="font-semibold">Booking Hold #{{ $p->booking?->booking_id ?? '-' }}</span>
            </div>
            <div class="flex items-center justify-between">
  ...
  <div class="grid lg:grid-cols-3 gap-5">
    <div class="lg:col-span-2 space-y-4">
      <div class="bg-white p-5 rounded-xl shadow border border-slate-100 space-y-4">
        <div class="flex items-center justify-between">
          <div class="font-semibold text-gray-800">Rincian Booking</div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          ...
          <div class="p-3 rounded-lg bg-slate-50">
            <div class="text-xs text-gray-500">Sumber</div>
            <div class="font-semibold text-gray-800">Booking Hold (ID: {{ $p->booking?->booking_id ?? '-' }})</div>
          </div>
          <div class="p-3 rounded-lg bg-slate-50">
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <div class="text-xs text-gray-500">Tinjau detail, beri catatan, lalu setujui atau tolak.</div>
            </div>
          </div>
    ...
    <div class="space-y-4">
      @if($status==='menunggu_verifikasi')
        ...
        <div class="bg-white p-5 rounded-xl shadow border border-slate-100">
          ...
          <form method="post" action="{{ route('takmir.verifikasi-booking.approve',$p) }}" class="space-y-3">
            @csrf
        ...
        <div class="bg-white p-5 rounded-xl shadow border border-slate-100">
          ...
          <form method="post" action="{{ route('takmir.verifikasi-booking.reject',$p) }}" class="space-y-3">
            @csrf
        ...
        @else
          <div class="bg-white p-5 rounded-xl shadow border border-slate-100 text-sm text-gray-700">Tidak ada aksi—status bukan menunggu verifikasi.</div>
        @endrole
      @else
        <div class="bg-white p-5 rounded-xl shadow border border-slate-100">
  ...
  <div class="pt-1 flex gap-3">
    <a href="{{ route('takmir.verifikasi-booking.index') }}" class="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300">Kembali ke daftar</a>
  </div>
</div>
@endsection
