            ...
            <div class="flex flex-wrap items-center gap-3">
              ...
              @if(in_array($b->status->value,['hold','proses']))
                <form method="post" action="{{ route('user.booking.update',$b) }}" onsubmit="return confirm('Batalkan booking ini?')">
                  ...
                  <button class="inline-flex items-center gap-2 rounded-full border border-rose-200 px-4 py-2 text-xs font-semibold text-rose-600 hover:border-rose-300 hover:text-rose-700">Batalkan</button>
                </form>
              @endif
            </div>
          </div>
