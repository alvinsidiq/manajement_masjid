...
it('admin can reject with reason', function(){
    ...
    actingAs($admin);
    post(route('admin.pemesanan.reject',$p), [
        'alasan_penolakan'=>'Tumpang tindih agenda.',
        'catatan'=>'Silakan ajukan tanggal lain.'
    ])->assertRedirect();

    $p->refresh();
    expect($p->status->value)->toBe('ditolak');
    expect($p->alasan_penolakan)->toBe('Tumpang tindih agenda.');
});
